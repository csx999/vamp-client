...
import vampdev.vampclient.gui.renderer.GuiDebugRenderer;
...
import vampdev.vampclient.gui.widgets.WRoot;
...
import vampdev.vampclient.utils.misc.CursorStyle;
...
public abstract class WidgetScreen extends Screen {
    ...
    public WidgetScreen(GuiTheme theme, String title) {
        ...
        this.parent = VampClient.mc.currentScreen;
    ...
    protected void init() {
        ...
        if (theme.hideHUD()) VampClient.mc.options.hudHidden = true;
        closed = false;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        double s = VampClient.mc.getWindow().getScaleFactor();
        mouseX *= s;
        mouseY *= s;
    ...
    public void onClose() {
        if (!locked) {
            ...
            if (theme.hideHUD() && !(parent instanceof WidgetScreen)) VampClient.mc.options.hudHidden = false;
    ...
    public void removed() {
        if (!closed) {
            ...
            if (onClose) {
                taskAfterRender = () -> {
                    locked = true;
                    VampClient.mc.setScreen(parent);
                };
            }
        }
    }
    ...
    private static class WFullScreenRoot extends WContainer implements WRoot {
        ...
        public boolean render(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
            if (!valid) {
                ...
                valid = true;
                mouseMoved(VampClient.mc.mouse.getX(), VampClient.mc.mouse.getY(), VampClient.mc.mouse.getX(), VampClient.mc.mouse.getY());
            }

...
public class AccountCache implements ISerializable<AccountCache> {
    ...
    public boolean loadHead(String url) {
        try {
            ...
            VampClient.LOG.error("Failed to read skin url (" + url + ").");
            return false;
        }
    }
    ...
    public static void loadSteveHead() {
        try {
            ByteBuffer data = TextureUtil.readResource(mc.getResourceManager().getResource(new Identifier("vamp", "textures/steve.png")).getInputStream());
            data.rewind();

...
import vampdev.vampclient.systems.modules.misc.Notebot;
...
public class NotebotCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(literal("help").executes(ctx -> {
            Util.getOperatingSystem().open("https://github.com/MeteorDevelopment/vamp/wiki/Notebot-Guide");
        ...
        builder.then(literal("play").then(argument("name", StringArgumentType.greedyString()).executes(ctx -> {
            ...
            Path path = VampClient.FOLDER.toPath().resolve(String.format("notebot/%s.txt", name));
            if (!path.toFile().exists()) {
                path = VampClient.FOLDER.toPath().resolve(String.format("notebot/%s.nbs", name));
            }

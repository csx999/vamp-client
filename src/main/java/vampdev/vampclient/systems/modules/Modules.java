...
package vampdev.vampclient.systems.modules;
...
import vampdev.vampclient.events.meteor.ActiveModulesChangedEvent;
...
import vampdev.vampclient.systems.modules.combat.*;
import vampdev.vampclient.systems.modules.misc.*;
import vampdev.vampclient.systems.modules.misc.swarm.Swarm;
import vampdev.vampclient.systems.modules.movement.*;
import vampdev.vampclient.systems.modules.movement.elytrafly.ElytraFly;
import vampdev.vampclient.systems.modules.movement.speed.Speed;
import vampdev.vampclient.systems.modules.player.*;
import vampdev.vampclient.systems.modules.render.*;
import vampdev.vampclient.systems.modules.render.hud.HUD;
import vampdev.vampclient.systems.modules.render.marker.Marker;
import vampdev.vampclient.systems.modules.render.search.Search;
import vampdev.vampclient.systems.modules.world.*;
import vampdev.vampclient.systems.modules.world.Timer;
import vampdev.vampclient.utils.Utils;
import vampdev.vampclient.utils.misc.ValueComparableMap;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                VampClient.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    private boolean onBinding(boolean isKey, int value) {
        if (moduleToBind != null && moduleToBind.keybind.canBindTo(isKey, value)) {
            ...
            VampClient.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
            moduleToBind = null;
            return true;
        }
        ...
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    VampClient.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    VampClient.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
...
//        add(new AntiAnchor());
    ...
    public static class ModuleRegistry extends Registry<Module> {
        public ModuleRegistry() {
            super(RegistryKey.ofRegistry(new Identifier("vamp", "modules")), Lifecycle.stable());
        }

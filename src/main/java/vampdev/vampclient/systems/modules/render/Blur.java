...
import vampdev.vampclient.events.render.RenderAfterWorldEvent;
import vampdev.vampclient.gui.WidgetScreen;
import vampdev.vampclient.renderer.Framebuffer;
import vampdev.vampclient.renderer.GL;
import vampdev.vampclient.renderer.PostProcessRenderer;
import vampdev.vampclient.renderer.Shader;
...
import vampdev.vampclient.eventbus.listeners.ConsumerListener;
...
public class Blur extends Module {
    ...
    public Blur() {
        ...
        VampClient.EVENT_BUS.subscribe(new ConsumerListener<>(WindowResizedEvent.class, event -> {
            if (fbo1 != null) {
                fbo1.resize();
                fbo2.resize();
            }
        ...
        VampClient.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
